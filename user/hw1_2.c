...
void print_recieved_char(int pid, char recieved) {
  printf("%d: recieved %c\n", pid, recieved);
}

/// Continue as parent process.
/// Ownership of write_fd and read_fd is taken.
void parent(int write_fd, int read_fd, char *data) {
  ...
  while (read(read_fd, &current_char, 1))
    print_recieved_char(my_pid, current_char);
  close(read_fd);
}
...
int main(int argc, char **argv) {
  ...
  if (fork_pid < 0) {
    ...
    child(pipe.forward.read_fd, pipe.backward.write_fd);
  } else { // fork_pid > 0
    close(pipe.forward.read_fd);
    close(pipe.backward.write_fd);
    parent(pipe.forward.write_fd, pipe.backward.read_fd, argv[1]);
  }
  exit(0);
}
